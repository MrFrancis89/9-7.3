...
import { abrirCalculadora, fecharCalculadora, calcDigito, calcSalvar } from './calculadora.js';
...
function adicionarFavorito() {
    ...
    mostrarConfirmacao('Adicionar "' + nome + '" à lista padrão?', () => {
        ...
        adicionarItem();
        mostrarToast('"' + nome + '" adicionado aos favoritos!');
    });
}
...
function mostrarNovidades() {
    ...
    fetch('./CHANGELOG.md')
        ...
        .then(md => {
            ...
            if (div) div.innerHTML = md.split('\n').slice(0, 40).map(l => '<p>' + l + '</p>').join('');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('calc-btn-teclado')?.addEventListener('click', () => {
        const inp = document.getElementById('novoQtd');
        fecharCalculadora();
    ...
    document.getElementById('lista-itens-container')?.addEventListener('input', e => {
        ...
        if (inp) { const d = coletarDadosDaTabela(); salvarDados(d); agendarSnapshot(); atualizarStatusSave(); verificarAlertas(); }
    });
    document.getElementById('lista-itens-container')?.addEventListener('blur', e => {
        const nome = e.target.closest('.nome-prod');
        if (nome) { salvarEAtualizar(); agendarSnapshot(); }
    }, true);
    ...
    // Parser de frações no campo de quantidade (blur)
    document.getElementById('lista-itens-container')?.addEventListener('blur', e => {
        const inp = e.target.closest('.input-qtd-tabela');
        if (inp && !inp.hasAttribute('readonly')) parseAndUpdateQuantity(inp);
    }, true);

    // Fechar calendar ao pressionar Escape
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape') { fecharCalendario(); fecharCalculadora(); }
    });
